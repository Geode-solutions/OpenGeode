...
def test_line_sphere_intersection():
    ...
    line1 = geometry.InfiniteLine2D(geometry.Segment2D(line1_origin, line1_dest))
    line2_origin = geometry.Point2D([0., 1.])
    line2_dest = geometry.Point2D([1., 1.])
    line2 = geometry.InfiniteLine2D(geometry.Segment2D(line2_origin, line2_dest))
    line3_origin = geometry.Point2D([0., 2.])
    line3_dest = geometry.Point2D([1., 2.])
    line3 = geometry.InfiniteLine2D(geometry.Segment2D(line3_origin, line3_dest))
    ...
    if not intersection_points1.has_intersection() or len(intersection_points1.result) != 2:
        raise ValueError("[Test] Wrong result for line_sphere_intersection2D with query line1" )
    ...
    if not intersection_points2.has_intersection() or len(intersection_points2.result) != 1:
        raise ValueError("[Test] Wrong result for line_sphere_intersection2D with query line 2" )
    if intersection_points2.result[0] != geometry.Point2D([2.0, 1.0]):
        raise ValueError("[Test] Wrong result for line_sphere_intersection2D with query line 2 (coords)" )

    intersection_points3 = geometry.line_sphere_intersection2D( line3, sphere )
    if intersection_points3.has_intersection():
        raise ValueError("[Test] Wrong result for line_sphere_intersection2D with query line 3" )
...
def test_segment_sphere_intersection():
    ...
    intersection_points1 = geometry.segment_sphere_intersection2D( segment1, sphere )
    if intersection_points1.has_intersection():
        raise ValueError("[Test] Wrong result for line_sphere_intersection2D with query segment 1" )

    intersection_points2 = geometry.segment_sphere_intersection2D( segment2, sphere )
    if not intersection_points2.has_intersection() or len(intersection_points2.result) != 1:
        raise ValueError("[Test] Wrong result for line_sphere_intersection2D with query segment 2" )

    intersection_points3 = geometry.segment_sphere_intersection2D( segment3, sphere )
    if not intersection_points3.has_intersection() or len(intersection_points3.result) != 1:
        raise ValueError("[Test] Wrong result for line_sphere_intersection2D with query segment 3" )

    intersection_points4 = geometry.segment_sphere_intersection2D( segment4, sphere )
    if not intersection_points4.has_intersection() or len(intersection_points4.result) != 2:
        raise ValueError("[Test] Wrong result for line_sphere_intersection2D with query segment 4" )

    intersection_points5 = geometry.segment_sphere_intersection2D( segment5, sphere )
    if intersection_points5.has_intersection():
        raise ValueError("[Test] Wrong result for line_sphere_intersection2D with query segment 5" )
...
def test_line_line_intersection():
    ...
    if intersection_point.has_intersection():
        raise ValueError("[Test] Wrong result for line_line_intersection2D with query lines 1 and 2" )
    ...
    if not intersection_point1.has_intersection() or intersection_point1.result != b:
        raise ValueError("[Test] Wrong result for line_line_intersection2D with query lines 1 and 3" )

    intersection_point2 = geometry.line_line_intersection2D( line1, line4 )
    if intersection_point2.has_intersection():
        raise ValueError("[Test] Wrong result for line_line_intersection2D with query lines 1 and 4" )
    ...
    if not intersection_point3.has_intersection() or intersection_point3.result != a:
        raise ValueError("[Test] Wrong result for line_line_intersection2D with query lines 1 and 5" )
...
def test_segment_segment_intersection():
    ...
    intersection_point = geometry.segment_segment_intersection2D( segment_cf, segment_de )
    if not intersection_point.has_intersection() or intersection_point.result != geometry.Point2D([2.5, 0.5]):
        raise ValueError("[Test] Wrong result for segment_segment_intersection2D with query segment_cf and segment_de" )

    intersection_point1 = geometry.segment_segment_intersection2D( segment_bc, segment_af )
    if not intersection_point1.has_intersection() or intersection_point1.result != b:
        raise ValueError("[Test] Wrong result for segment_segment_intersection2D with query segment_bc and segment_af" )

    intersection_point2 = geometry.segment_segment_intersection2D( segment_cd, segment_df )
    if not intersection_point2.has_intersection() or intersection_point2.result != d:
        raise ValueError("[Test] Wrong result for segment_segment_intersection2D with query segment_cd and segment_df" )

    intersection_point3 = geometry.segment_segment_intersection2D( segment_ab, segment_cd )
    if intersection_point3.has_intersection():
        raise ValueError("[Test] Wrong result for segment_segment_intersection2D with query segment_ab and segment_cd" )

    intersection_point4 = geometry.segment_segment_intersection2D( segment_ab, segment_ab2 )
    if intersection_point4.has_intersection():
        raise ValueError("[Test] Wrong result for segment_segment_intersection2D with query segment_ab and segment_ab2" )

    intersection_point5 = geometry.segment_segment_intersection2D( segment_ab, segment_ef )
    if intersection_point5.has_intersection():
        raise ValueError("[Test] Wrong result for segment_segment_intersection2D with query segment_ab and segment_ef" )

    intersection_point6 = geometry.segment_segment_intersection2D( segment_bc, segment_df )
    if intersection_point6.has_intersection():
        raise ValueError("[Test] Wrong result for segment_segment_intersection2D with query segment_bc and segment_df" )
...
def test_segment_line_intersection():
    ...
    intersection_point = geometry.segment_line_intersection2D( segment_de, line_cf )
    if not intersection_point.has_intersection() or intersection_point.result != geometry.Point2D([2.5, 0.5]):
        raise ValueError("[Test] Wrong result for segment_line_intersection2D with query segment_de and line_cf" )

    intersection_point1 = geometry.segment_line_intersection2D( segment_ef, line_df )
    if not intersection_point1.has_intersection() or intersection_point1.result != f:
        raise ValueError("[Test] Wrong result for segment_line_intersection2D with query segment_ef and line_df" )

    intersection_point2 = geometry.segment_line_intersection2D( segment_cd, line_ef )
    if intersection_point2.has_intersection():
        raise ValueError("[Test] Wrong result for segment_line_intersection2D with query segment_cd and line_ef" )

    intersection_point3 = geometry.segment_line_intersection2D( segment_ab, line_ef )
    if intersection_point3.has_intersection():
        raise ValueError("[Test] Wrong result for segment_line_intersection2D with query segment_ab and line_ef" )

    intersection_point4 = geometry.segment_line_intersection2D( segment_ab, line_ce )
    if intersection_point4.has_intersection():
        raise ValueError("[Test] Wrong result for segment_line_intersection2D with query segment_ab and line_ce" )
...
def test_segment_plane_intersection():
    ...
    if intersect.has_intersection():
        raise ValueError("[Test] Wrong result for segment_plane_intersection3D with query segment_de" )

    intersect1 = geometry.segment_plane_intersection3D( segment_da, plane )
    if intersect1.has_intersection():
        raise ValueError("[Test] Wrong result for segment_plane_intersection3D with query segment_da" )

    intersect2 = geometry.segment_plane_intersection3D( segment_fg, plane )
    if intersect2.has_intersection():
        raise ValueError("[Test] Wrong result for segment_plane_intersection3D with query segment_fg" )

    intersect3 = geometry.segment_plane_intersection3D( segment_hi, plane )
    if intersect3.result != a:
        raise ValueError("[Test] Wrong result for segment_plane_intersection3D with query segment_hi" )
    ...
    if intersect7.result != b:
        raise ValueError("[Test] Wrong result for segment_plane_intersection3D with query segment_kb" )

    intersect8 = geometry.segment_plane_intersection3D( segment_mn, plane )
    if intersect8.has_intersection():
        raise ValueError("[Test] Wrong result for segment_plane_intersection3D with query segment_mn" )
...
def test_segment_triangle_intersection():
    ...
    intersection = geometry.segment_triangle_intersection3D( segment_de, triangle )
    if intersection.has_intersection():
        raise ValueError("[Test] Wrong result for segment_triangle_intersection3D with query segment_de" )

    intersection1 = geometry.segment_triangle_intersection3D( segment_da, triangle )
    if intersection1.has_intersection():
        raise ValueError("[Test] Wrong result for segment_triangle_intersection3D with query segment_da" )

    intersection2 = geometry.segment_triangle_intersection3D( segment_fg, triangle )
    if intersection2.has_intersection():
        raise ValueError("[Test] Wrong result for segment_triangle_intersection3D with query segment_fg" )

    intersection3 = geometry.segment_triangle_intersection3D( segment_hi, triangle )
    if intersection3.result != a:
        raise ValueError("[Test] Wrong result for segment_triangle_intersection3D with query segment_hi" )

    intersection4 = geometry.segment_triangle_intersection3D( segment_hj, triangle )
    if intersection4.result != geometry.Point3D([0.5, 0.5, 0.0]):
        raise ValueError("[Test] Wrong result for segment_triangle_intersection3D with query segment_hj" )

    intersection5 = geometry.segment_triangle_intersection3D( segment_ki, triangle )
    if intersection5.result != geometry.Point3D([0.5, 0.25, 0.0]):
        raise ValueError("[Test] Wrong result for segment_triangle_intersection3D with query segment_ki" )

    intersection6 = geometry.segment_triangle_intersection3D( segment_li, triangle )
    if intersection6.has_intersection():
        raise ValueError("[Test] Wrong result for segment_triangle_intersection3D with query segment_li" )

    intersection7 = geometry.segment_triangle_intersection3D( segment_kb, triangle )
    if intersection7.result != b:
        raise ValueError("[Test] Wrong result for segment_triangle_intersection3D with query segment_kb" )

    intersection8 = geometry.segment_triangle_intersection3D( segment_mn, triangle )
    if intersection8.has_intersection():
        raise ValueError("[Test] Wrong result for segment_triangle_intersection3D with query segment_mn" )
